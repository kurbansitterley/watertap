...
To register the flow type, we use the ``register_flow_type`` method of the flowsheet costing block.
...
Adding the units is important to include in the cost expression to ensure that the costing calculations are done with the correct units.
...
Assume that we have a mass-based flow of "bazchem" in our unit model that we want a cost for called ``m.fs.unit.bazchem_flow_mass`` that is in kg/min.
...
Frequently these steps are done within a unit model costing method; see :ref:`how to create a custom costing method<how_to_create_custom_costing_method>` for an example of how to cost this flow within a unit model costing method.