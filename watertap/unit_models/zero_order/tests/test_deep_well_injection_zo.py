...
def test_costing():
    ...
    m.fs.unit.inlet.flow_mass_comp[0, "H2O"].fix(0.04381)
    m.fs.unit.inlet.flow_mass_comp[0, "sulfur"].fix(0.10)
    m.fs.unit.inlet.flow_mass_comp[0, "toc"].fix(0.20)
    m.fs.unit.inlet.flow_mass_comp[0, "tss"].fix(0.30)
    m.fs.unit.load_parameters_from_database()
    m.fs.unit.pipe_distance[0].fix(10)  # overwrite default value of zero
